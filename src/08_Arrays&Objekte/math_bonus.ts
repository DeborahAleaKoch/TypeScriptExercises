...
const minTemp: number[] = weather.data.map(
	(element) => element.temperature_min
);
const maxTemp: number[] = weather.data.map(
	(element) => element.temperature_max
);

const minimalTemp = Math.min(...minTemp);
const maximalTemp = Math.max(...maxTemp);

console.log(`Die niedrigste Temperatur beträgt: ${minimalTemp} °C!`);
console.log(`Die höchste Temperatur beträgt: ${maximalTemp} °C!`);

// ----- Die Temeperaturen werden von der kleinsten Zahl bis zur größten Zahl sortiert und anders herum.
console.log(
	minTemp.slice().sort((a, b) => {
		if (a > b) {
			return a - b;
		} else {
			return b - a;
...
// ---- Die Windgeschwindigkeit wird aus dem Object Array rausgezogen

const windSpeed: number[] = weather.data.map((element) => element.windspeed);
console.log("Windgeschwindigkeit:", windSpeed);
console.log(Math.max(...windSpeed));
console.log(Math.min(...windSpeed));
