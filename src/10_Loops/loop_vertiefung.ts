...
function gapAfter100signs(text: string) {
	let textInBlocksOfHundred: string[] = [];
	const totalLines = Math.ceil(text.length / 100);
	console.log(totalLines);

	let newIndexTextSign: number = 0;

	for (let i = 1; i <= totalLines; i++) {
		const line: string = text.slice(newIndexTextSign, newIndexTextSign + 100);
...
// console.log(gapAfter100signs(text));
